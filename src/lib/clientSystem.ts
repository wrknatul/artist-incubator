...
const ARCHETYPE_PRESETS: Record<ClientArchetype, { 
  ...
  dreamer: {
    ...
    templates: {
      ...
      angry: 'Это вообще не то, что я представлял... Хотя я сам не знаю что представлял...',
  ...
  micromanager: {
    ...
    templates: {
      ...
      angry: 'Почему вы отступили от ТЗ? Я ясно написал: 14px, а не 16px!',
  ...
  scrooge: {
    ...
    templates: {
      ...
      angry: 'За ТАКИЕ деньги — ЭТО?! Другие берут дешевле и делают лучше!',
  ...
  ghost: {
    ...
    templates: {
      ...
      angry: 'Хм. Я думал будет по-другому. Ну ладно.',
...
const REVIEW_TEMPLATES: Record<ClientArchetype, { good: string[]; bad: string[] }> = {
  dreamer: {
    ...
    bad: ['Ну... не знаю... Не то. Вроде красиво, но не то 😕', 'Я представлял это совсем по-другому...'],
  },
  micromanager: {
    good: ['Профессионалы. Чётко следовали ТЗ. Рекомендую.', 'Все пункты ТЗ выполнены. Качественная работа.'],
    bad: ['Не следуют указаниям. Приходилось повторять 10 раз.', 'Отклонение от ТЗ в 15 местах. Неприемлемо.'],
  },
  scrooge: {
    good: ['Хорошо сделали, но дороговато 💸', 'Нормально. Хотя за эти деньги ожидал большего.'],
    bad: ['За такие деньги — позорное качество. Больше не обращусь.', 'Переплатил. Результат не стоит этих денег.'],
  },
  ghost: {
    good: ['О, нормально получилось! Сорри что пропадал 😅', 'Ого, а неплохо! Спасибо за терпение.'],
    bad: ['Хм. Я думал будет по-другому. Ну ладно.', 'Не совсем то, но мне лень разбираться...'],
  },
  professional: {
    good: ['Отличная команда, соблюдали дедлайны, качество на высоте.', 'Профессиональный подход. Буду обращаться снова.'],
    bad: ['Есть замечания по архитектуре. Дедлайн нарушен. Средний уровень.', 'Качество ниже ожиданий. Рекомендации по улучшению отправлю.'],
  },
  chameleon: {
    good: ['Вот ЭТО — то что нужно! Наконец-то поняли мою идею!', 'Круто! Хотя завтра я могу передумать 😄'],
    bad: ['Это не то что я хотел! Хотя вчера хотел именно это...', 'Нет-нет-нет, всё переделать! Хотя... ладно, сойдёт.'],
  },
  toxic: {
    good: ['Ну... сойдёт. Хотя могло быть лучше.', 'Нормально. Не ожидал, если честно.'],
    bad: ['КОШМАР. Не работайте с ними. Берут деньги ни за что!', 'Мой племянник сделал бы лучше. За бесплатно.'],
  },
};
