...
1. СООТВЕТСТВИЕ ЗАКАЗУ (самый важный критерий!):
   ...
   - Тема правильная, но тип сайта другой (просили магазин — сделали лендинг) → [ОЦЕНКА: 1-2]
   - Тема правильная, тип правильный, но много ошибок/пустых секций → [ОЦЕНКА: 2-3]
   - Соответствует заказу, но есть замечания по дизайну/функционалу → [ОЦЕНКА: 3-4]
   - Полностью соответствует заказу, хороший дизайн → [ОЦЕНКА: 4-5]

2. КАЧЕСТВО:
   - Есть ли все запрошенные секции?
   - Работает ли навигация?
   - Адекватный ли дизайн для отрасли?
   - Есть ли реальный контент или placeholder-текст?

ВАЖНО: Если тематика сайта НЕ СОВПАДАЕТ с заказом — ЭТО АВТОМАТИЧЕСКИ 1 ЗВЕЗДА, без исключений!
Пример: заказали криптокошелёк, а сделали кофейню → 1 звезда.

${honestyNote}
Поставь оценку СТРОГО по критериям в формате [ОЦЕНКА: X] (от 1 до 5).
Объясни свою оценку в 2-3 предложениях.`;
      }
    } else {
      // Legacy fallback
